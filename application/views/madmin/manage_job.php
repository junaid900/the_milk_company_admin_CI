...
we_work_with_on_and_off_trade_leaders_to_inspire_creativity_and_sophistication.'); ?></p>
    	...
    	<div class="row col-md-12">
         ...
         <div class="">
            <button class="btn btn-primary" onclick="location.href='<?=base_url()."".admin_ctrl()."/add_job"?>'"><i class="fa fa-plus"></i>&nbsp;<?= $page_title ?></button>
        </div>	
    </div>
                        ...
                        <table class="dataTables-example table-striped">
                            ...
                            foreach ($table_data as $data) {$i++; ?>
                                <tr class="gradeX row_<?php echo $data['status']; ?>"
                                    id="<?php echo $data['job_id']; ?>">
                                    <td><?= $i ?></td>
                                    <td><img src="<?php echo base_url(); ?><?= $data['image'] ?> "  width="50px"  alt="" /></td>
                                    <td><?= $data['position'] ?></td>
                                        ...
                                        <div class="table-toggle">
                                            <div class="toggle-btn1 <?php if ($data['status'] == 'Active') {
                                                ...
                                                <input type="checkbox" class="cb-value"
                                                       value="<?php echo $data['job_id']; ?>" <?php if ($data['status'] == 'Active') {
                                                    echo 'checked';
                                                } ?>/>
                                    ...
                                    <td class="center">
                                        <a href="<?=base_url().admin_ctrl().'/edit_job/'.$data['job_id']; ?>">
                                            <img src="<?=base_url(); ?>assets/admin/img/edit.png" class="mr_1" /> 
                                        </a>
                                        <a href="javascript:;"
                                           onclick="confirm_modal_action('<?php echo base_url() . admin_ctrl() ?>/<?= $page_name ?>/delete/<?php echo $data['job_id']; ?>');">
