                        ...
                        <table class="dataTables-example table-striped">
                            ...
                            foreach ($table_data as $data) {$i++; ?>
                                <tr class="gradeX row_<?php echo $data['status']; ?>"
                                    id="<?php echo $data['products_category_id']; ?>">
                                    <td><?= $i ?></td>
                                    <td><?= $data['en_name'] ?></td>
                                    <td>
                                        <div class="table-toggle">
                                            <div class="toggle-btn1 <?php if ($data['status'] == 'Active') {
                                                ...
                                                <input type="checkbox" class="cb-value"
                                                       value="<?php echo $data['products_category_id']; ?>" <?php if ($data['status'] == 'Active') {
                                                    echo 'checked';
                                                } ?>/>
                                    ...
                                    <td class="center">
                                        <a href="<?=base_url().admin_ctrl().'/edit_category/'.$data['products_category_id']; ?>">
                                            <img src="<?=base_url(); ?>assets/admin/img/edit.png" class="mr_1" /> 
                                        </a>
                                        <a href="javascript:;"
                                           onclick="confirm_modal_action('<?php echo base_url() . admin_ctrl() ?>/<?= $page_name ?>/delete/<?php echo $data['products_category_id']; ?>');">
